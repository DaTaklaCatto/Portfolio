class CutCornerSVG extends HTMLElement {
  ...
  getMeasureElement() {
    ...
    if (!helperContainer) {
      ...
      helperContainer.style.cssText = "position:absolute;visibility:hidden;pointer-events:none;height:0;width:0;overflow:hidden;";
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === "width") {
      ...
      name === "fill" // Update SVG when fill changes
    ) {
      this.updateSVG();
    } else if (name !== "class") {
  ...
  updateSVG() {
    ...
    this.svg.setAttribute("viewBox", `${0} ${0} ${Math.ceil(this._width) - strokeWidth} ${Math.ceil(this._height) - strokeWidth}`);
  ...
  get cutCornerRadius() {
    // Choose attribute based on mobile/desktop
    const attrBaseName = this.isMobile && this.hasAttribute("corner-rounded-mobile") ? "corner-rounded-mobile" : "corner-rounded";
  ...
  get cutSize() {
    // Choose attribute based on mobile/desktop
    const attrName = this.isMobile && this.hasAttribute("corner-size-mobile") ? "corner-size-mobile" : "corner-size";
